...
inline bool isGreater(double x, double y)
{
  ...
  return std::abs(x - y) >0.00;
}
int main()
{
    int duration,n;
    float price,payment;
    while(scanf("%d %f %f %d",&duration,&payment,&price,&n) && duration>=0)
    {
        float iR[duration+3];
        mem(iR,0.00);
        int x;
        ...
        loan = price;
        price = price+payment;
        float pay = loan/(float)(duration);
        price=price - ((price*iR[0])/100.00);
        //cout<<price<<" "<<loan<<endl;
        while(price<=loan)
        {
            price = price - ((price*iR[counter])/100.00);
            loan = loan-pay;
            counter++;
            //cout<<price<<" "<<loan<<endl;
        }
