...
{
    int r,c,s;
    while(scanf("%d%d%d",&r,&c,&s)==3 && r+s+c>0)
    {
        int table[r+1][c+1];
        char tab[r+1][c+1];
        mem(table,0);
        ...
        if(currPosC==0)
            currPosC=c;
        //cout<<currPosR<<"<------->"<<currPosC<<endl;
        int counter=0;
        int flag=0;
        int value;
        while(!flag)
        {
            if(currPosR>r || currPosC>c|| currPosR<1 || currPosC<1)
            {
                //cout<<"dhukc\n";
                flag=2;
                break;
            }
            if(table[currPosR][currPosC]==0)
            {
                counter++;
                table[currPosR][currPosC]=counter;
                if(tab[currPosR][currPosC]=='N')
                {
                    currPosR--;
                }
                else if(tab[currPosR][currPosC]=='S')
                {
                    currPosR++;
                }
                else if(tab[currPosR][currPosC]=='E')
                {
                    currPosC++;
                }
                else if(tab[currPosR][currPosC]=='W')
                {
                    currPosC--;
                }
            }
            ...
            {
                value=table[currPosR][currPosC]-1;
                flag=1;
                break;
            }
            //cout<<currPosR<<" "<<currPosC<<endl;
